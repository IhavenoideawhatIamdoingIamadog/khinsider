...
if __name__ == '__main__':
    ...
    requiredModules = [
        ...
        ['CloudScraper', 'cloudscraper', 'cloudscraper >= 1.0.0']
    ]
...
def friendlyDownloadFile(file, path, index, total, verbose=True):
    ...
    if not os.path.exists(path):
        ...
        for triesElapsed in range(3):
            ...
            try:
                file.download(path)
            # This probably won't be activated because cloudscraper doesn't have ConnectionError
            # Just keeping it like this so the Python kernel doesn't throw a fit
...
class Song(object):
    ...
    * files: A list of the song's files - there may be several if the song
    ...
    @lazyProperty
    def _soup(self):
        r = scraper.get(self.url, timeout=10, headers=headers)
...
def search(term):
    ...
    name results.
    """
    r = scraper.get(urljoin(BASE_URL, 'search'), params={'search': term}, headers=headers)
